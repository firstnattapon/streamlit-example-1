...
# ฟังก์ชันใหม่สำหรับ NVTS (fix = 2100)
@lru_cache(maxsize=128)
def sell_nvts(asset, fix_c=2100, Diff=60):
    if asset == 0:
        return 0, 0, 0
    s1 = (fix_c - Diff) / asset
    s2 = round(s1, 2)
...
@lru_cache(maxsize=128)
def buy_nvts(asset, fix_c=2100, Diff=60):
    if asset == 0:
        return 0, 0, 0
    b1 = (fix_c + Diff) / asset
    b2 = round(b1, 2)
...
# Pre-calculate all buy/sell values - แก้ไขให้ NVTS ใช้ฟังก์ชันพิเศษ
calculations = {}
assets = [x_3, x_4, x_5, x_6, x_7, x_8, x_9]
asset_names = ['NEGG', 'FFWM', 'RIVN', 'APLS', 'NVTS', 'QXO', 'RXRX']
...
for i, (asset, name) in enumerate(zip(assets, asset_names)):
    if name == 'NVTS':
        # ใช้ฟังก์ชันพิเศษสำหรับ NVTS (fix = 2100)
        calculations[name] = {
            'sell': sell_nvts(asset, Diff=x_2),
            'buy': buy_nvts(asset, Diff=x_2)
        }
    else:
        # ใช้ฟังก์ชันปกติสำหรับหุ้นอื่น (fix = 1500)
        calculations[name] = {
            'sell': sell(asset, Diff=x_2),
            'buy': buy(asset, Diff=x_2)
        }
...
if st.button("RERUN"):
    ...
    # Clear lru_cache - เพิ่มฟังก์ชันใหม่
    sell.cache_clear()
    buy.cache_clear()
    sell_nvts.cache_clear()
    buy_nvts.cache_clear()
