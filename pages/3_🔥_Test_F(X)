...
# [NEW] เพิ่ม state สำหรับ Global Diff
...
with tab2:
    # ---------- [NEW] Header row: nex_day | Min_Rebalance | Global Diff ----------
    ...
    st.write("---")
    # [NEW] ลบ st.sidebar.number_input('Diff') ออกไปแล้ว
...
with tab1:
    ...
    # [NEW] ตรรกะการคำนวณที่รองรับ per-ticker diff
    ...
    for config in ASSET_CONFIGS:
        ...
        # --- LOGIC CHANGE ---
        # ใช้ diff จาก config ถ้ามี, ถ้าไม่มีให้ใช้ global_diff
    ...
    for config in configs_to_display:
        ...
        # [NEW] ดึงค่า diff ที่ถูกต้องสำหรับ Ticker นี้อีกครั้งเพื่อส่งไปแสดงผล
        ...
        trading_section(
            ...
            diff=ticker_diff_for_display, # [NEW] ส่ง diff ที่ถูกต้อง
