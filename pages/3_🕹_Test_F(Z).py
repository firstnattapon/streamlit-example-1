...
# ---------------------------------------------------------------------------------
# Fetch all data — รองรับ fast rerun (ตัด Optimistic UI ออก)
# ---------------------------------------------------------------------------------
@st.cache_data(ttl=300, show_spinner=False)
...
# ---------------------------------------------------------------------------------
# Update controls (ลบ Optimistic UI)
# ---------------------------------------------------------------------------------
def render_asset_update_controls(configs: List[Dict], clients: Dict[int, thingspeak.Channel]) -> None:
    ...
    - ไม่มี Optimistic UI: ไม่ override ค่าใน session
    - หลังสำเร็จ bump cache_bump และ rerun เพื่อดึงค่าจริง
    """
...
def trading_section(
    ...
    # SELL — HTTP GET + Fast focus (no optimistic override)
    ...
    if col3.checkbox(f'sell_match_{ticker}'):
        if col3.button(f"GO_SELL_{ticker}"):
            try:
                ...
                if resp.strip() == "0":
                    col3.error("ThingSpeak ไม่บันทึกค่า (resp=0): ตรวจ Write API Key/field และเว้น ~15s/ช่อง")
                else:
    ...
    # BUY — HTTP GET + Fast focus (no optimistic override)
    ...
    if col6.checkbox(f'buy_match_{ticker}'):
        if col6.button(f"GO_BUY_{ticker}"):
            try:
                ...
                resp = ts_update_with_rate_limit(write_key, field_name, new_asset_val, channel_id=channel_id, min_interval=16.0, max_wait=8.0)
                if resp.strip() == "0":
                    col6.error("ThingSpeak ไม่บันทึกค่า (resp=0): ตรวจ Write API Key/field และเว้น ~15s/ช่อง")
                else:
...
# ดึงข้อมูลหลัก — ไม่มีการใช้ optimistic cache override
