...
with tab2:
    ...
    # ดึงค่า Diff จาก state มาใช้ (จะถูกอัปเดตโดย logic ใน tab1)
...
with tab1:
    ...
    # --- คำนวณสัญญาณล่วงหน้าสำหรับทุก Ticker ---
    ...
    # === 💡 GOAL_1: เพิ่มตัวเลือก filter เมื่อ nex == 1 ===
    ...
    if selected_ticker != st.session_state.get('_last_selected_ticker'):
        new_diff = None
        # หา config ของ ticker ที่เลือก (เฉพาะกรณีที่เลือก Ticker จริงๆ ไม่ใช่ Filter)
        ...
        # Rerun เพื่อให้ Sidebar number_input อัปเดตค่าทันที
        if new_diff is not None:
            st.rerun()
    # === 💡 GOAL_1: DYNAMIC DIFF LOGIC END ===

    # === 💡 GOAL_1: สร้าง Logic การกรองการแสดงผล ===
    ...
    # --- คำนวณค่า buy/sell สำหรับทุก Ticker โดยใช้ Diff ที่อัปเดตแล้ว ---
    ...
    for config in ASSET_CONFIGS:
        ...
        # ใช้ค่า diff จาก state ที่เป็นไดนามิกแล้ว
        current_diff = float(st.session_state.get('diff_value', 60))
        calculations[ticker] = {
            'sell': sell(asset_value, fix_c=fix_c, Diff=current_diff),
            'buy': buy(asset_value, fix_c=fix_c, Diff=current_diff),
        }

    # --- วนลูปแสดงผลเฉพาะ Ticker ที่ผ่านการกรอง ---
    for config in configs_to_display:
        ...
        trading_section(
            ...
            diff=float(st.session_state.get('diff_value', 60)), # ส่งค่า diff ที่ถูกต้อง
