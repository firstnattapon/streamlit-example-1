...
# --- UI Components (MODIFIED render_asset_inputs function) ---
    ...
    for i, config in enumerate(configs):
        with cols[i]:
            ...
            if config.get('option_config'):
                ...
                # แยก ticker และข้อมูล level เป็น 2 บรรทัด
                original_label = config['option_config']['label']
                # แยก ticker และส่วนที่เหลือ
                if '(' in original_label and ')' in original_label:
                    ticker_part = original_label.split('(')[0]
                    level_part = '(' + original_label.split('(')[1]
                    label = f"{ticker_part}\n{level_part}"
                else:
                ...
                # สำหรับ NEGG ที่ไม่มี option_config
