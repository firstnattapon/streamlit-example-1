...
def find_best_solution_ga(prices_window: np.ndarray, population_size: int, generations: int, mutation_rate: float = 0.01) -> Tuple[float, np.ndarray]:
    ...
    for _ in range(generations):
        ...
        offspring[:, 0] = 1 # Ensure first action is always 1
...
def generate_actions_sliding_window_ga(ticker_data: pd.DataFrame, window_size: int, population_size: int, generations: int, **kwargs) -> Tuple[np.ndarray, pd.DataFrame]:
    prices = ticker_data['Close'].to_numpy(); n = len(prices)
    final_actions = np.array([], dtype=np.int32); window_details_list = []
    num_windows = (n + window_size - 1) // window_size
    progress_bar = st.progress(0, text="กำลังประมวลผล Genetic Algorithm Sliding Windows...")
    st.write(f"🧬 Evolving Solutions using GA")
...
def display_comparison_charts(results: Dict[str, pd.DataFrame], chart_title: str = '📊 เปรียบเทียบกำไรสุทธิ (Net Profit)'):
    ...
    if not valid_dfs:
        st.warning("ไม่มีข้อมูล 'net' สำหรับสร้างกราฟ"); return
...
def render_ga_test_tab():
    ...
    st.info("กลยุทธ์นี้ใช้วิวัฒนาการเชิงคำนวณ (GA) เพื่อ 'พัฒนา' Action Sequence ที่ดีที่สุดในแต่ละ Window แทนการสุ่ม")
    if st.button("🚀 เริ่มทดสอบ Best Genetic Algorithm", type="primary", key="ga_test_button"):
        ...
        with st.spinner("กำลังคำนวณกลยุทธ์ต่างๆ (GA Search)..."):
            actions_ga, df_windows = generate_actions_sliding_window_ga(
                ...
                st.session_state.ga_population_size, 
        ...
        csv = df_windows.to_csv(index=False)
        st.download_button(
            label="📥 ดาวน์โหลด GA Window Details (CSV)",
            data=csv,
            file_name=f'best_ga_{ticker}_{st.session_state.window_size}w.csv',
            mime='text/csv'
        )
...
def render_analytics_tab():
    ...
    if st.session_state.df_for_analysis is not None:
        ...
        try:
            # Check for multiple possible file formats
            required_cols_options = [
                ['window_number', 'timeline', 'max_net', 'best_seed', 'price_change_pct', 'action_sequence', 'window_size'], # Random Seed
                ['window_number', 'timeline', 'max_net', 'best_seed', 'r_param', 'price_change_pct', 'action_sequence'], # Chaotic Seed
                ['window_number', 'timeline', 'max_net', 'price_change_pct', 'action_count', 'action_sequence'] # GA
            ]
            valid_file = False
    ...
    if st.button("📈 เปรียบเทียบประสิทธิภาพ Seeds", type="primary", key="compare_manual_seeds_btn"):
        ...
        with st.spinner("กำลังดึงข้อมูลและจำลองการเทรด..."):
            ...
            for i, line_info in enumerate(st.session_state.manual_seed_lines):
                ...
                rng_best = np.random.default_rng(input_seed); full_actions = rng_best.integers(0, 2, size=size_seed); actions_from_tail = full_actions[-tail_seed:].tolist()
                ...
                if not df_line.empty:
                    df_line.index = ticker_data.index[:sim_len]; results[f"Seed {input_seed} (Tail {tail_seed})"] = df_line; max_sim_len = max(max_sim_len, sim_len)
...
def main():
    ...
    with tab4:
        render_ga_test_tab() # เรียกใช้ฟังก์ชันของ Tab ใหม่
    with tab5:
    ...
    with st.expander("📖 คำอธิบายวิธีการทำงานและแนวคิดการ Refactor"):
        ...
        **หลักการ Refactoring ที่ใช้ในเวอร์ชันนี้:**
        - **Numba JIT Compilation**: เพิ่ม decorator `@njit` ให้กับฟังก์ชันที่คำนวณหนักๆ เพื่อเร่งความเร็ว
        - **Wrapper Functions**: สร้างฟังก์ชัน Wrapper (เช่น `calculate_optimized_cached`) เพื่อให้โค้ดเดิมที่เรียกใช้งานไม่ต้องเปลี่ยนแปลงตามไปด้วย
        - **Non-Invasive Feature Addition**: การเพิ่มฟีเจอร์ใหม่ (Chaotic Seed, Genetic Algorithm) จะทำโดยการสร้างฟังก์ชันและ UI แยกต่างหาก และเลียนแบบโครงสร้างเดิม เพื่อไม่ให้กระทบกับส่วนที่ทำงานได้ดีอยู่แล้ว

        **กลยุทธ์ที่เพิ่มเข้ามา:**
        - **🌀 Best Seed (Chaotic):** แทนที่จะสุ่ม Seed แบบปกติ จะใช้สมการ Logistic Map เพื่อสร้าง Action Sequence ที่ดูเหมือนจะสุ่มแต่มีรูปแบบที่ซับซ้อนกว่า โดยจะค้นหาพารามิเตอร์ `r` และ `x0` ที่ดีที่สุด
        - **🧬 Best Seed (Genetic Algo):** ใช้วิธีการแบบ Genetic Algorithm เพื่อ "วิวัฒนาการ" ชุดคำตอบที่ดีที่สุดขึ้นมาในแต่ละ Window โดยเลียนแบบการคัดเลือกตามธรรมชาติ, การผสมพันธุ์, และการกลายพันธุ์
        """)
