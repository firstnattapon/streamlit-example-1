...
def find_best_chaotic_seed(prices_window: np.ndarray, num_seeds_to_try: int, max_workers: int) -> Tuple[int, float, np.ndarray]:
    ...
    if window_len < 2:
        ...
        return default_seed, 0.0, np.ones(window_len, dtype=int)
    ...
    seed_list = [chaotic_params_to_seed(r, x0) for r, x0 in zip(rng_for_params.uniform(3.57, 4.0, num_seeds_to_try), rng_for_params.uniform(0.01, 0.99, num_seeds_to_try))]
...
def display_comparison_charts(results: Dict[str, pd.DataFrame], chart_title: str = '📊 เปรียบเทียบกำไรสุทธิ (Net Profit)'):
    ...
    for name, df in results.items():
        if not df.empty:
            # Reindex and fill forward to handle different simulation lengths
...
def main():
    ...
    with st.expander("📖 คำอธิบายวิธีการทำงานและแนวคิดการ Refactor"):
        ...
        **หลักการ Refactoring ที่ใช้ในเวอร์ชันนี้:**
        ...
        - **Modular Strategies**: เพิ่มกลยุทธ์ `Chaotic Seed` ใหม่โดยสร้างฟังก์ชันเฉพาะ (`find_best_chaotic_seed`, `generate_actions_sliding_window_chaotic`) และแท็บ UI ของตัวเอง (`render_chaotic_test_tab`) โดยยังคงใช้โครงสร้างหลักและฟังก์ชันคำนวณผลตอบแทนร่วมกันได้
        """)
