...
# --- START: MODIFICATION FOR SESSION STATE ---
# 1. Initialize session state if it doesn't exist to prevent errors on the first run.
...
if Nex_day_:
    ...
    # 2. Update the session state when a button is clicked, not a local variable.
    if nex_col.button("Nex_day"):
        st.session_state.nex = 1
        st.session_state.Nex_day_sell = 0 # Reset the sell flag
    ...
    # 3. Always display the value from the session state.
    st.write(f"nex value = {st.session_state.nex}", f" | Nex_day_sell = {st.session_state.Nex_day_sell}" if st.session_state.Nex_day_sell else "")
else:
    # Optional: Reset the state if the main checkbox is unchecked.
    st.session_state.nex = 0
    st.session_state.Nex_day_sell = 0
# --- END: MODIFICATION FOR SESSION STATE ---
...
for config in ASSET_CONFIGS:
    ...
    action_emoji = ""
    try:
        # 4. Use the session state values in the logic.
        if not df_data.empty and df_data.action.values[1 + st.session_state.nex] != "":
            ...
            final_action = 1 - raw_action if st.session_state.Nex_day_sell == 1 else raw_action
            if final_action == 1:
    ...
    # 5. Pass the session state values to the trading function.
