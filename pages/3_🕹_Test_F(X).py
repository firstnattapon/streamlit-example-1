...
def generate_actions_perfect_foresight(price_arr: np.ndarray, fix: int = 1500) -> np.ndarray:
    ...
    actions = np.zeros(n, dtype=np.int32); best_final_day = np.argmax(dp)
    ...
    if st.session_state.df_for_analysis is not None:
        ...
        try:
            required_cols_options = [
                ['window_number', 'timeline', 'max_net', 'best_seed', 'price_change_pct', 'action_sequence'],
                ['window_number', 'timeline', 'max_net', 'best_seed', 'r_param', 'x0_param', 'price_change_pct', 'action_sequence'],
                ['window_number', 'timeline', 'max_net', 'price_change_pct', 'action_count', 'action_sequence']
            ]
            valid_file = False
            for req_cols in required_cols_options:
                if all(col in df_to_analyze.columns for col in req_cols): valid_file = True; break
            if not valid_file: st.error(f"ไฟล์ CSV ไม่สมบูรณ์! กรุณาตรวจสอบว่ามีคอลัมน์ที่จำเป็นครบถ้วน"); return
...
def render_manual_seed_tab(config: Dict[str, Any]):
    ...
    with st.container(border=True):
        ...
        action_sequence_str = st.text_area(
            "ป้อน Action Sequence (ในรูปแบบ List ของ 0 และ 1):",
            value=st.session_state.manual_action_sequence,
            height=100,
            key="manual_action_input"
        )
    ...
    if st.button("📈 เริ่มทดสอบ Forward Test", type="primary", key="forward_test_btn"):
        ...
        try:
            ...
            if not isinstance(actions_to_test, list) or not all(isinstance(i, int) and i in [0, 1] for i in actions_to_test):
                st.error("รูปแบบ Action Sequence ไม่ถูกต้อง! ต้องเป็น List ของ 0 และ 1 เท่านั้น เช่น [1, 0, 1]")
                return
        except (ValueError, SyntaxError):
            st.error("รูปแบบ Action Sequence ไม่ถูกต้อง! กรุณาตรวจสอบวงเล็บและเครื่องหมาย comma")
            return
        ...
        with st.spinner("กำลังดึงข้อมูลและจำลองการเทรด..."):
            ...
            if not df_manual.empty:
                df_manual.index = ticker_data.index[:len(df_manual)]
                results["Forward Test Sequence"] = df_manual
            ...
            if final_results_list:
                order = [Strategy.PERFECT_FORESIGHT, "Forward Test Sequence", Strategy.REBALANCE_DAILY]
                final_results_list.sort(key=lambda x: order.index(x['name']) if x['name'] in order else 99)
...
def main():
    ...
    with st.expander("📖 คำอธิบายวิธีการทำงานและแนวคิด"):
        ...
        **กลยุทธ์ที่ใช้ในการค้นหา:**
        - **🚀 Best Seed (Random):** เป็นวิธีที่พื้นฐานที่สุด ค้นหาโดยการสุ่ม Action Sequence ขึ้นมาจำนวนมาก (ควบคุมโดย `seed`) แล้วเลือกอันที่ให้ผลตอบแทนดีที่สุด
        - **🌀 Best Seed (Chaotic):** ใช้ "Logistic Map" ในการสร้าง Action Sequence ที่มีรูปแบบซับซ้อนแต่กำหนดได้จากพารามิเตอร์ `r` และ `x0`
        - **🧬 Best Seed (Genetic Algo):** เลียนแบบวิวัฒนาการทางธรรมชาติ โดยการสร้าง "ประชากร" ของ Action Sequences แล้วคัดเลือก, ผสมพันธุ์, และกลายพันธุ์ เพื่อ "พัฒนา" ให้ได้ Sequence ที่ดีที่สุด
        
        **แท็บอื่นๆ:**
        - **📊 Advanced Analytics:** ใช้วิเคราะห์ไฟล์ผลลัพธ์ (.csv) ที่ดาวน์โหลดจากการทดสอบ เพื่อดูภาพรวมและจำลองการเทรดจริงด้วย "Stitched DNA"
