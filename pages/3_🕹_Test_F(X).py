...
class Strategy:
    ...
    OPTIMIZED_MA_CROSSOVER = "Optimized MA Crossover Sliding Window"
...
def load_config(filepath: str = "dynamic_seed_config.json") -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "assets": ["FFWM", "NEGG", "RIVN", "BTC-USD", "ETH-USD"], # เพิ่มสินทรัพย์ตัวอย่าง
            "default_settings": {
                "selected_ticker": "FFWM", "start_date": "2025-06-10", "window_size": 30, 
                "num_seeds": 30000, "max_workers": 8, "ga_population_size": 50, "ga_generations": 20
            },
...
# 3.1 Standard Seed Generation (No Changes)
...
# 3.2 Chaotic Seed Generation (No Changes)
...
# ==============================================================================
# 3.3 [NEW] Genetic Algorithm Generation
# ==============================================================================
def find_best_solution_ga(prices_window: np.ndarray, population_size: int, generations: int, mutation_rate: float = 0.01) -> Tuple[float, np.ndarray]:
    window_len = len(prices_window)
    if window_len < 2:
        return 0.0, np.ones(window_len, dtype=np.int32)
    
    rng = np.random.default_rng()

    # 1. สร้างประชากรเริ่มต้น
    population = rng.integers(0, 2, size=(population_size, window_len), dtype=np.int32)
    population[:, 0] = 1 # บังคับ action แรกเป็น 1

    for gen in range(generations):
        # 2. ประเมิน Fitness
        fitness_scores = []
        for chromosome in population:
            # ใช้ lru_cache เพื่อเร่งความเร็วในการคำนวณ fitness ที่ซ้ำซ้อน
        ...
        # 3. คัดเลือก (Selection) - เลือก 50% ที่ดีที่สุดมาเป็นพ่อแม่
        num_parents = population_size // 2
        parent_indices = np.argsort(fitness_scores)[-num_parents:]
        parents = population[parent_indices]
        
        # 4. สร้างลูกหลาน (Crossover)
        ...
        for k in range(num_offspring):
            parent1_idx = rng.integers(0, num_parents)
            parent2_idx = rng.integers(0, num_parents)
            crossover_point = rng.integers(1, window_len) # ไม่เอา index 0
        ...
        offspring[:, 0] = 1 # ตรวจสอบว่า action แรกเป็น 1 เสมอ

        # สร้างประชากรรุ่นใหม่
        population[:num_parents] = parents
        population[num_parents:] = offspring

    # ค้นหาคำตอบที่ดีที่สุดในประชากรสุดท้าย
    final_fitness = []
    for chromosome in population:
        _, sumusd, _, _, _, refer = calculate_optimized_cached(tuple(chromosome), tuple(prices_window))
        net = sumusd[-1] - refer[-1] - sumusd[0] if len(sumusd) > 0 else -np.inf
    ...
    best_net = final_fitness[best_idx]
    best_actions = population[best_idx]

    return best_net, best_actions
...
def generate_actions_sliding_window_ga(ticker_data: pd.DataFrame, window_size: int, population_size: int, generations: int) -> Tuple[np.ndarray, pd.DataFrame]:
    ...
    st.write(f"🧬 Evolving Solutions using Genetic Algorithm")
...
# ==============================================================================
# 3.4 [NEW] Optimized Moving Average Crossover Generation
# ==============================================================================

@njit(cache=True)
def generate_actions_ma_crossover_numba(prices: np.ndarray, short_win: int, long_win: int) -> np.ndarray:
    n = len(prices)
    if n < long_win:
        actions = np.ones(n, dtype=np.int32)
        return actions

    # คำนวณ MA แบบ manual ใน Numba
    short_ma = np.empty(n, dtype=np.float64)
    long_ma = np.empty(n, dtype=np.float64)
    short_ma[:short_win-1] = np.nan
    long_ma[:long_win-1] = np.nan
    
    for i in range(short_win - 1, n):
        short_ma[i] = np.mean(prices[i - short_win + 1 : i + 1])
    for i in range(long_win - 1, n):
        long_ma[i] = np.mean(prices[i - long_win + 1 : i + 1])
    ...
    # สร้าง Action จากการเปลี่ยนของสัญญาณ
    actions = np.zeros(n, dtype=np.int32)
    actions[0] = 1 # Action แรกเสมอ
    for i in range(long_win, n):
        if signal[i] == 1 and signal[i-1] == 0: # Crossover up
            actions[i] = 1
            
    return actions

def find_best_ma_params_for_window(prices_window: np.ndarray) -> Tuple[float, np.ndarray, int, int]:
    window_len = len(prices_window)
    if window_len < 4: # ต้องการอย่างน้อย 4 วันเพื่อให้มี short/long MA ที่มีความหมาย
        return 0.0, np.ones(window_len, dtype=np.int32), 1, 2
    ...
    # กำหนดช่วงการค้นหา
    short_range = range(2, min(window_len // 2, 16)) # short MA ไม่เกิน 15 วัน หรือครึ่งหนึ่งของ window
    long_range = range(4, min(window_len, 41)) # long MA ไม่เกิน 40 วัน
    ...
    for short_w in short_range:
        for long_w in long_range:
            ...
            actions = generate_actions_ma_crossover_numba(prices_window, short_w, long_w)
    ...
    return best_net, best_actions, best_short, best_long


def generate_actions_sliding_window_ma(ticker_data: pd.DataFrame, window_size: int) -> Tuple[np.ndarray, pd.DataFrame]:
    prices = ticker_data['Close'].to_numpy(); n = len(prices)
    final_actions = np.array([], dtype=np.int32); window_details_list = []
    num_windows = (n + window_size - 1) // window_size
    progress_bar = st.progress(0, text="กำลังประมวลผล Optimized MA Crossover Sliding Windows...")
    st.write(f"📈 Optimizing MA Crossover Strategy")
    st.write(f"📊 ข้อมูล: {n} วัน | Window: {window_size} วัน")
    st.write("---")
    ...
    for i, start_index in enumerate(range(0, n, window_size)):
        ...
        max_net, best_actions, best_short, best_long = find_best_ma_params_for_window(prices_window)
        final_actions = np.concatenate((final_actions, best_actions))
...
# ==============================================================================
# 4. UI Rendering Functions (Updated for New Models)
# ==============================================================================
...
def run_and_display_strategy(
    strategy_func, 
    strategy_args: Dict,
    strategy_name: str, 
    ticker_data: pd.DataFrame, 
    df_columns_to_show: List[str]):
    
    if st.button(f"🚀 เริ่มทดสอบ ({strategy_name})", type="primary", key=f"btn_{strategy_name}"):
        if st.session_state.start_date >= st.session_state.end_date:
        ...
        st.download_button(
            label=f"📥 ดาวน์โหลด {strategy_name} Details (CSV)",
            data=csv,
            file_name=f'{strategy_name.replace(" ", "_")}_{st.session_state.test_ticker}_{st.session_state.window_size}w.csv',
            mime='text/csv'
        )
...
def render_test_tab():
    ...
    if ticker_data.empty:
        st.error(f"ไม่พบข้อมูลสำหรับ {ticker} ในช่วงวันที่ที่เลือก"); return
...
def render_ma_test_tab():
    st.write("---")
    st.markdown("### 📈 ทดสอบด้วย Optimized Moving Average Crossover")
    st.info("กลยุทธ์นี้จะค้นหาคู่เส้นค่าเฉลี่ย (Short & Long MA) ที่ให้ผลกำไรสูงสุดจากการเกิดสัญญาณ Crossover ในแต่ละ Window")
    ...
    columns_to_show = ['window_number', 'timeline', 'max_net', 'best_short_ma', 'best_long_ma', 'price_change_pct', 'action_count']
    run_and_display_strategy(generate_actions_sliding_window_ma, args, Strategy.OPTIMIZED_MA_CROSSOVER, ticker_data, columns_to_show)


# -- Other Tabs (Analytics, Manual) are kept as they are --
    ...
    if st.session_state.df_for_analysis is not None:
        ...
        try:
            required_cols_options = [
                ['window_number', 'timeline', 'max_net', 'best_seed', 'price_change_pct', 'action_sequence', 'window_size'],
                ['window_number', 'timeline', 'max_net', 'r_param', 'x0_param', 'price_change_pct', 'action_sequence', 'window_size'],
                ['window_number', 'timeline', 'max_net', 'best_short_ma', 'best_long_ma', 'price_change_pct', 'action_sequence', 'window_size'],
                ['window_number', 'timeline', 'max_net', 'price_change_pct', 'action_sequence', 'window_size']
            ]

            valid_file = False
            for req_cols in required_cols_options:
                if all(col in df_to_analyze.columns for col in req_cols):
                    valid_file = True
                    break
            
            if not valid_file:
                 st.error(f"ไฟล์ CSV ไม่สมบูรณ์! กรุณาตรวจสอบว่ามีคอลัมน์ที่จำเป็นครบถ้วน")
                 return
...
# ==============================================================================
# 5. Main Application (Updated with New Tabs)
# ==============================================================================
def main():
    ...
    with st.expander("📖 คำอธิบายกลยุทธ์และแนวคิด"):
        ...
        *   **🎲 Random Seed (Tab: Random Seed):** เป็นวิธีที่พื้นฐานที่สุด ค้นหาโดยการสุ่ม Action Sequence ขึ้นมาจำนวนมาก (ควบคุมโดย `seed`) แล้วเลือกอันที่ให้ผลตอบแทนดีที่สุด
        
        *   **🌀 Chaotic Seed (Tab: Chaotic Seed):** ซับซ้อนขึ้นมาเล็กน้อย โดยใช้สมการคณิตศาสตร์ที่เรียกว่า "Logistic Map" ในการสร้าง Action Sequence ที่ดูเหมือนจะสุ่มแต่จริงๆ แล้วมีรูปแบบที่เกิดจากพารามิเตอร์ `r` และ `x0` กลยุทธ์นี้จะค้นหาพารามิเตอร์คู่ที่ดีที่สุด
        
        *   **🧬 Genetic Algorithm (Tab: GA Search):** **[โมเดลใหม่]** เป็นเทคนิคการค้นหาที่เลียนแบบวิวัฒนาการทางธรรมชาติ มันจะสร้าง "ประชากร" ของ Action Sequences ขึ้นมา แล้วคัดเลือก, ผสมพันธุ์, และกลายพันธุ์ เพื่อ "พัฒนา" ให้ได้ Sequence ที่ดีที่สุดในแต่ละรุ่น (Generation) เป็นวิธีที่ฉลาดกว่าการสุ่มธรรมดา
        
        *   **📈 Optimized Moving Average Crossover (Tab: MA Crossover):** **[โมเดลใหม่]** เปลี่ยนแนวคิดจากการค้นหา Action Sequence ตรงๆ มาเป็นการค้นหา "พารามิเตอร์ของกฎ" แทน กลยุทธ์นี้จะทดลองใช้เส้นค่าเฉลี่ย (Moving Average) ระยะสั้นและระยะยาวหลายๆ ค่า เพื่อหาว่าคู่ไหนที่สร้างสัญญาณซื้อขาย (Crossover) แล้วให้ผลกำไรดีที่สุดในหน้าต่างนั้นๆ ซึ่งเป็นแนวทางที่ใกล้เคียงกับนักวิเคราะห์ทางเทคนิค
