...
def render_hybrid_multi_mutation_tab():
    ...
    with st.expander("📖 คำอธิบายวิธีการทำงานและแนวคิด (Multi-Mutation)"):
        st.markdown(
            ...
            2.  **เฟส 2: กระบวนการ "กลายพันธุ์ต่อเนื่อง" (Iterative Mutation)**
                *   นำ **"แชมป์เปี้ยนปัจจุบัน"** มาเป็นต้นแบบ และพยายามพัฒนายีนส์ต่อตามจำนวนรอบ (`num_mutations`) ที่กำหนด
                *   **ในแต่ละรอบ:** จะมีการค้นหารูปแบบการกลายพันธุ์ (Mutation Pattern) ที่ดีที่สุดเพื่อสร้าง **"ผู้ท้าชิง"**
                *   หากผู้ท้าชิงทำกำไรได้ดีกว่าแชมป์เปี้ยนปัจจุบัน ผู้ท้าชิงจะกลายเป็น **"แชมป์เปี้ยนคนใหม่"** และถูกนำไปเป็นต้นแบบในรอบถัดไป
            ...
            #### 🔬 เจาะลึกกระบวนการกลายพันธุ์ (Mutation)
            
            การกลายพันธุ์คือการ "เปลี่ยนแปลง" Actions ของแชมป์เปี้ยนปัจจุบันเล็กน้อยอย่างสุ่ม เพื่อมองหาโอกาสในการปรับปรุง โดยแต่ละ `mutation_seed` จะสร้าง "รูปแบบ" การกลายพันธุ์ที่ไม่ซ้ำกัน ซึ่งมีขั้นตอนดังนี้:
            
            1.  **สร้าง "หน้ากากกลายพันธุ์" (Mutation Mask):**
                *   `mutation_mask = mutation_rng.random(window_len) < mutation_rate`
                *   โปรแกรมจะสร้างชุดตัวเลขสุ่ม (ค่าระหว่าง 0.0 ถึง 1.0) ขึ้นมาให้มีจำนวนเท่ากับจำนวนวันใน Window
                *   จากนั้นจะเปรียบเทียบเลขสุ่มแต่ละตัวกับ `mutation_rate` (เช่น 5% หรือ 0.05)
                *   ผลลัพธ์ที่ได้คือ "หน้ากาก" ที่เป็น `True` (กลายพันธุ์) ในตำแหน่งที่เลขสุ่มน้อยกว่า `mutation_rate` และเป็น `False` (คงเดิม) ในตำแหน่งอื่นๆ
            
            2.  **พลิกค่า Actions ตามหน้ากาก:**
                *   `final_mutated_actions[mutation_mask] = 1 - final_mutated_actions[mutation_mask]`
                *   โปรแกรมจะนำ Actions ของแชมป์เปี้ยนมาคัดลอก จากนั้นจะเลือกเฉพาะวันที่ "หน้ากาก" เป็น `True` แล้วทำการ **"พลิกค่า"** Action ในวันนั้นๆ
                *   *ถ้าเดิมเป็น `1` (ซื้อ) จะถูกเปลี่ยนเป็น `0` (ถือ)*
                *   *ถ้าเดิมเป็น `0` (ถือ) จะถูกเปลี่ยนเป็น `1` (ซื้อ)*

            3.  **บังคับใช้กฎข้อแรก:**
                *   `final_mutated_actions[0] = 1`
                *   เพื่อให้แน่ใจว่าการจำลองเริ่มต้นด้วยการซื้อเสมอ โปรแกรมจะกำหนดให้ Action ของวันแรกสุดเป็น `1` เสมอ หลังจากกระบวนการกลายพันธุ์เสร็จสิ้น
            
            > อัลกอริทึมจะทดสอบ "รูปแบบการกลายพันธุ์" แบบนี้นับพันครั้ง (ตาม `num_seeds`) เพื่อหารูปแบบที่ดีที่สุดเพียงหนึ่งเดียวที่จะนำไปท้าชิงกับแชมป์เปี้ยนปัจจุบัน
            """
        )
