...
checkbox1 = st.checkbox('Delta / Max.Sum_Buffer %' , value=1 )
if checkbox1 :
    Delta_2 = Delta
    Delta_2['FFWM'] =  ((Delta.FFWM_net_pv.values) / (float(1500 + (abs( np.min(Sum_Buffer.FFWM_Buffer.values))+ abs( np.max(Sum_Buffer.FFWM_Buffer.values)))))) *100
    Delta_2['NEGG'] =  ((Delta.NEGG_net_pv.values)  / (float(1500 + (abs( np.min(Sum_Buffer.NEGG_Buffer.values))+ abs( np.max(Sum_Buffer.NEGG_Buffer.values)))))) *100
    ...
    st.write( abs( np.min(Sum_Buffer.Sum_Buffer.values))+ abs( np.max(Sum_Buffer.Sum_Buffer.values))+1500 )
    st.write((Delta.Sum_Delta.values)  / float (1500 + (abs( np.min(Sum_Buffer.Sum_Buffer.values))+ abs( np.max(Sum_Buffer.Sum_Buffer.values)))) *100 )
