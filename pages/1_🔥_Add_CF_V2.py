...
def display_results(metrics: Dict[str, float], options_pl: float, config: Dict[str, Any]):
    ...
    with st.expander("📈 Results", expanded=True):
        ...
        st.metric(label=f"💰 Baseline {metrics['log_pv'] -  metrics['ln']  :,.1f } - {config.get('product_cost_default', 0)} f"{=+2750 }" ", value=f"{metrics['net_cf'] - config.get('cashflow_offset', 0.0):,.2f}")
