...
def display_results(metrics: Dict[str, float], options_pl: float, config: Dict[str, Any]):
    ...
    st.metric(label=f"💰 Baseline {metrics['log_pv'] - metrics['ln']  :,.2f}{config.get('cashflow_offset', 0.0):,.2f}   @ {config.get('cashflow_offset_comment', '')}", value=f"{metrics['net_cf'] - config.get('cashflow_offset', 0.0):,.2f}")
