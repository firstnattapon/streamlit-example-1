...
if checkbox1 :
    ...
    Delta_2['{}'.format(Ticker_s[3])] =  (Delta['{}_net_pv'.format(Ticker_s[3])].values /(abs(np.min( Buffer['{}_Buffer'.format(Ticker_s[3])].values)) +  abs(np.max( Buffer['{}_Buffer'.format(Ticker_s[3])].values)) ) ) *100
    ...
    st.write('Delta_Add.Risk' , add_risk , 1500 / add_risk , 100 /   Delta_2['{}'.format(Ticker_s[3])].values[-1] )
    ...
    st.write(survival ,(abs(np.min( Buffer['{}_Buffer'.format(Ticker_s[3])].values)) +  abs(np.max( Buffer['{}_Buffer'.format(Ticker_s[3])].values)) ) )
# except:pass
