...
def render_settings_tab():
    ...
    c1, c2 = st.columns(2)
    st.session_state.test_ticker = c1.selectbox("เลือก Ticker สำหรับทดสอบ", options=asset_list, index=asset_list.index(st.session_state.test_ticker) if st.session_state.test_ticker in asset_list else 0, help="Ticker นี้ใช้สำหรับการอ้างอิงในการตั้งค่าเริ่มต้นเท่านั้น การรันจริงจะทำกับทุก Ticker ใน Config")
    ...
    # --- MOVED: Generate Encoded String from Window Result ---
    # This section appears only after a simulation has been run in the other tab.
    if 'df_windows_details' in st.session_state and not st.session_state.get('df_windows_details', pd.DataFrame()).empty:
        st.markdown("#### 🎁 Generate Encoded String from Window Result")
        st.info(f"ข้อมูลนี้มาจากผลการรันของ Ticker: **{st.session_state.get('last_processed_ticker', 'N/A')}**. \n\nเลือกหมายเลข Window จากตารางผลลัพธ์ในแท็บ `🧬 Hybrid (Multi-Mutation)` เพื่อสร้าง Encoded String")
        ...
        with c1:
            ...
            try:
                ...
                # Correctly calculate action length for the selected window
        ...
        with c2:
            ...
            if st.button("Encode Selected Window", key="window_encoder_button"):
                try:
                    ...
                    mutation_rate_int = int(st.session_state.mutation_rate * 10) if st.session_state.mutation_rate < 1.0 else int(st.session_state.mutation_rate)
...
def render_hybrid_multi_mutation_tab():
    ...
    with st.expander("📖 คำอธิบายวิธีการทำงานและแนวคิด (Multi-Mutation)"):
        # ... (Content of expander remains the same) ...
        st.markdown("...") 
        st.code("...", language="python")

    # --- NEW BATCH PROCESSING BUTTON ---
    if st.button(f"🚀 Run Batch for ALL Tickers & Find Best Seeds", type="primary"):
        ...
        if not all_tickers:
            st.error("ไม่พบรายการ Ticker ในไฟล์ `hybrid_seed_config.json`"); return

        encoded_results = {}
        status_text = st.empty()
        
        for i, ticker in enumerate(all_tickers):
            status_text.info(f"🔄 Processing Ticker {i+1}/{len(all_tickers)}: {ticker}...")
            try:
                ticker_data = get_ticker_data(ticker, str(st.session_state.start_date), str(st.session_state.end_date))
                if ticker_data.empty:
                    encoded_results[ticker] = "Error: No data found"
                    continue
                ...
                if df_windows.empty:
                    encoded_results[ticker] = "Error: Simulation failed"
                    continue
                
                # Find best window and encode its parameters
                best_window_row = df_windows.loc[df_windows['final_net'].idxmax()]
                best_window_index = df_windows['final_net'].idxmax()

                dna_seed = int(best_window_row['dna_seed'])
                mutation_seeds_str = best_window_row['mutation_seeds']
                mutation_seeds = []
                if mutation_seeds_str not in ["None", "[]"]:
                    cleaned_str = mutation_seeds_str.strip('[]')
                    if cleaned_str:
                ...
                start_idx = best_window_index * st.session_state.window_size
                action_length = min(st.session_state.window_size, len(ticker_data) - start_idx)
                ...
                encoded_results[ticker] = encoded_string

                # --- Save results of the CURRENT ticker for detailed display ---
                prices = ticker_data['Close'].to_numpy()
                ...
                for name, df in st.session_state.simulation_results.items():
                ...
                st.session_state.last_processed_ticker = ticker
            
            except Exception as e:
                st.error(f"เกิดข้อผิดพลาดระหว่างประมวลผล {ticker}: {e}")
                encoded_results[ticker] = f"Error: {e}"

        status_text.success("✅ Batch process completed!")
        st.write("### 🔑 Encoded Strings for Best Window of Each Ticker")
        st.json(encoded_results)
        st.session_state.run_completed = True

    # --- Display results section (shows results for the LAST processed ticker) ---
    if 'run_completed' in st.session_state:
        st.success(f"การทดสอบเสร็จสมบูรณ์! กำลังแสดงผลลัพธ์แบบละเอียดสำหรับ Ticker: **{st.session_state.get('last_processed_ticker', 'N/A')}**")
        ...
        if not df_windows.empty:
            ...
            ticker = st.session_state.get('last_processed_ticker', 'TICKER')
