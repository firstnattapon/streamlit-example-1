...
# --- NEW HELPER FUNCTION ---
def format_asset_label(label_str: str) -> str:
    """
    Formats a label string into a two-line HTML string for Streamlit Markdown.
    e.g., "AGL(LV:15@1.83)" -> "AGL<br>(LV:15@1.83)"
    e.g., "NEGG_ASSET" -> "NEGG_ASSET<br>(NULL)"
    """
    if not label_str:
        return "(NULL)"
    split_pos = label_str.find('(')
    if split_pos != -1:
        part1 = label_str[:split_pos].strip()
        part2 = label_str[split_pos:].strip()
        return f"**{part1}**<br>{part2}"
    else:
        return f"**{label_str}**<br>(NULL)"

# --- MODIFIED FUNCTION ---
def render_asset_inputs(configs: List[Dict], last_assets: Dict) -> Dict[str, float]:
    """
    Renders the asset input widgets in the 'Controls' tab with two-line labels.
    """
    asset_inputs = {}
    ...
    for i, config in enumerate(configs):
        with cols[i]:
            ...
            # Format the label and display it using Markdown
            formatted_label = format_asset_label(raw_label)
            st.markdown(formatted_label, unsafe_allow_html=True)

            # Render the number_input with an empty label
            if config.get('option_config'):
                option_val = config['option_config']['base_value']
                real_val = st.number_input(
                    label="", # Use empty label
                    step=0.001,
                    value=last_val,
