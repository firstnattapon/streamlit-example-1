...
# --- START: REFINED FUNCTION (Code has been updated here) ---
def render_charts(config: Dict[str, Any]):
    """Renders ThingSpeak charts using iframe components by iterating through a defined list."""
    st.divider()
    ...
    if not main_channel_id:
        st.warning("Main channel ID not found in config. Cannot display charts.")
        return

    # --- Define all charts in one place for easy management ---
    # You can now easily reorder, add, or remove charts by editing this list.
    charts_to_display = [
        {'key': 'net_cf', 'title': 'Cashflow'},
        {'key': 'now_pv', 'title': 'Current Total Value'},
        {'key': 'pure_alpha', 'title': 'Pure_Alpha'},
        {'key': 'cost_minus_cf', 'title': 'Product_cost - CF'},
        {'key': 'buffer', 'title': 'Buffer'},
    ]

    for chart_info in charts_to_display:
        field_name = main_fields_map.get(chart_info['key'])
        if field_name:
            chart_number = field_name.replace('field', '')
            url = (f'https://thingspeak.com/channels/{main_channel_id}/charts/{chart_number}'
                   f'?bgcolor=%23ffffff&color=%23d62020&dynamic=true&results=60&type=line&update=15')

            st.write(f"**{chart_info['title']}**")
            components.iframe(url, width=800, height=200, scrolling=False)
            st.divider()
        else:
            st.warning(f"Field key '{chart_info['key']}' not found in config. Skipping '{chart_info['title']}' chart.")
