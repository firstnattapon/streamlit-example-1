...
def display_nk_breakdown(nk: Dict[str, Any]):
    ...
    with st.expander("N vs K Breakdown (แยกค่า N/K + สัดส่วน + K premium)", expanded=False):
        ...
        col1.write("Σ N (Stocks)", f"{n_total:,.0f}")
        col2.write("Σ K_Value (Options @Break-even)", f"{kv_total:,.0f}")
        col3.write("Σ K (Premium, cost)", f"{kp_total:,.0f}")  # likely negative
        ...
        col4.write("สัดส่วน: K_Value/N | |K|/N", f"{r1_txt} | {r2_txt}")
...
# --- 3.5 NEW: N-K BREAKDOWN FUNCTIONS (ADDED) ---
