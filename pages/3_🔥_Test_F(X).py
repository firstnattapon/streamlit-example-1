...
def display_results(metrics: Dict[str, float], options_pl: float, total_option_cost: float, config: Dict[str, Any]):
    ...
    with st.expander("📈 Results", expanded=True):
        ...
        offset_display_val = -config.get('cashflow_offset', 0.0)  # will be 0 with normalized baseline
    ...
    with st.expander("Show 'ln_weighted' Calculation Breakdown"):
        st.write("ค่า `ln_weighted` คำนวณมาจากผลรวมของหุ้นแต่ละตัว (หลัง Normalize fix_c ให้รวมเท่ากับ Product_cost):")
        ...
        for item in ln_breakdown_data:
            if item['ref_price'] > 0:
                formula_string = (
                    ...
                    f"[ {item['fix_c']:.4f} * ln( {item['live_price']:.2f} / {item['ref_price']:.2f} ) ]"
                )
            else:
...
# --- 4. CALCULATION FUNCTION (MODIFIED: normalize fix_c to product_cost) ---
...
def calculate_metrics(stock_assets: List[Dict[str, Any]], option_assets: List[Dict[str, Any]], user_inputs: Dict[str, Any], config: Dict[str, Any]) -> Tuple[Dict[str, float], float, float]:
    ...
    # Options P/L: intrinsic-only, same as before
    ...
    # --- Normalize fix_c weights so that their sum equals product_cost ---
    sum_fix_c = sum(asset.get('fix_c', 0.0) for asset in stock_assets)
    scale = (product_cost / sum_fix_c) if sum_fix_c > 0 else 1.0

    log_pv_baseline, ln_weighted = 0.0, 0.0
    ln_breakdown = []

    for asset in stock_assets:
        base_fix_c = asset.get('fix_c', 0.0)
        fix_c = base_fix_c * scale  # normalized weight
        ticker = asset['ticker'].strip()
        ...
        ln_breakdown.append({
            "ticker": ticker,
            "fix_c": fix_c,               # show normalized fix_c
            "live_price": live_price,
            "ref_price": ref_price,
            "contribution": contribution
        })

    metrics['log_pv_baseline'] = log_pv_baseline              # should equal product_cost
...
def main():
    ...
    # 1) Calculate with normalized baseline
    metrics, options_pl, total_option_cost = calculate_metrics(stock_assets, option_assets, user_inputs, config)

    # 2) Dynamic Cashflow Offset becomes zero by construction (keep field for compatibility)
    #    We keep the line so downstream UI remains unchanged.
    config['cashflow_offset'] = max(0.0, config.get('cashflow_offset', 0.0)) * 0.0
