...
# --- Helper: Detect option type robustly (CALL/PUT) --------------------------
def normalize_option_type(opt: Dict[str, Any]) -> str:
    """
    Return 'call' or 'put'.
    Priority:
      1) explicit field 'option_type'
      2) infer from name string containing 'put'/'call'
      3) default -> 'call'
    """
    t = str(opt.get("option_type", "")).strip().lower()
    if t in ("call", "put"):
        return t
...
# --- Initialize 'portfolio_cash' in session_state using the fetch function ---
...
def calculate_metrics(stock_assets: List[Dict[str, Any]], option_assets: List[Dict[str, Any]], user_inputs: Dict[str, Any], config: Dict[str, Any]) -> Tuple[Dict[str, float], float, float]:
    ...
    Calculates all core metrics. Options P/L now supports both CALL and PUT.
      - Long/Short handled via sign of `contracts_or_shares`
      - CALL intrinsic: max(0, last - strike) * contracts
      - PUT  intrinsic: max(0, strike - last) * contracts
      - Cost basis: contracts * premium (credit if short)
    """
    metrics = {}
