...
TIMEFRAME_OPTIONS = {
    ...
    "4h": {"minutes": 240, "yfinance_interval": "1h"},
...
def encode_number_stream(numbers: List[int]) -> str:
    return "".join(f"{len(str(int(num)))}{int(num)}" for num in numbers)
...
def get_ticker_data(ticker: str, start_date: str, end_date: str, timeframe: str = "1d") -> pd.DataFrame:
    try:
        ...
        data = yf.Ticker(ticker).history(start=start_date, end=end_date, interval=interval)[['Close']]
...
class SimulationTracer:
    ...
    def decode_number_stream(encoded_string: str) -> List[int]:
        if not isinstance(encoded_string, str) or not encoded_string.isdigit():
            raise ValueError("Input must be a digit-only string")
        decoded_numbers = []
        idx = 0
        while idx < len(encoded_string):
            try:
                ...
                raise ValueError(f"Invalid encoded string at position {idx}")
    ...
    def _decode_and_set_attributes(self):
        ...
        if len(dna_numbers) < 3:
            raise ValueError("DNA string must include length, rate, and DNA seed")
...
def render_settings_tab():
    ...
    st.session_state.selected_timeframe = st.selectbox(
        ...
        help="Use 1d, 4h, 1h, or 30m bars for simulation and timeline encoding"
    )
    ...
    with st.expander("🔌 **Global Encoding & Tracing Settings** (Advanced)", expanded=True):
        ...
        st.session_state.trace_start_timestamp = st.number_input(
            ...
            format="%d",
            help="Cloud decode uses this Unix timestamp plus timeframe to map current time to the DNA action index"
        )
...
def render_tracer_tab():
    ...
    with meta_col1:
        manual_timeframe_input = st.selectbox(
            ...
            key="enc_timeframe",
            help="Used only when Unix Start Timestamp is greater than 0"
        )
    with meta_col2:
        manual_start_timestamp_input = st.number_input(
            "Unix Start Timestamp (0 = legacy/no timeline metadata)",
            min_value=0,
            value=0,
            format="%d",
            key="enc_start_ts"
