    ...
    def fetch_monitor(asset_config: Dict) -> Tuple[str, Tuple[pd.DataFrame, str, Optional[datetime.date]]]:
        ...
        try:
            ...
            try:
                ...
                # ใช้ .apply() เพื่อกำหนด action ตามค่า index column
                def get_action_for_row(row_index_val):
                    try:
                        idx = int(row_index_val)
                        if 0 <= idx < len(final_actions):
                            return int(final_actions[idx])
                        return ""
                    except (ValueError, TypeError):
                        return ""
                
                df['action'] = df['index'].apply(get_action_for_row)
            except Exception as e:
