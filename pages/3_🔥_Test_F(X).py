...
#                 st.info("Performance analysis of the portfolio's net value, comparing worst-case scenarios, average performance, and potential gains.")

#                 net_series = df_sumusd_['net']

#                 # --- Daily CF ---
#                 daily_changes = net_series.diff()
#                 min_daily_cf = daily_changes.min()
#                 avg_daily_cf = daily_changes.mean()
...
#                         avg_30_day_cf = rolling_30_day_change.mean()
...
#                         avg_90_day_cf = rolling_90_day_change.mean()
...
#                 with col2:
#                     st.subheader("Average CF")
#                     st.metric(label="📊 1-Day CF (Avg. Day)", value=f"{avg_daily_cf:,.2f} USD")
#                     st.metric(label="📊 30-Day CF (Avg. Month)", value=f"{avg_30_day_cf:,.2f} USD")
#                     st.metric(label="📊 90-Day CF (Avg. Quarter)", value=f"{avg_90_day_cf:,.2f} USD")

#                 with col3:
#                     st.subheader("Gain Potential")
#                     st.metric(label="📈 Trough-to-Peak Gain", value=f"{trough_to_peak_gain:,.2f} USD", help="Maximum gain from the portfolio's lowest value to a subsequent peak.")
