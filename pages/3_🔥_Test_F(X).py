...
def get_prices_batch(tickers: List[str]) -> Dict[str, float]:
    # ดึงราคาเป็นชุดเดียว (ลด call ซ้ำ ๆ)
    try:
        ...
        for t in tickers:
            try:
                ser = df[t]['Close'].dropna() if len(tickers) > 1 else df['Close'].dropna()
...
def get_history_df_close_bkk(ticker: str, start_date: Optional[str]) -> pd.DataFrame:
    # ดึงเท่าที่ต้องใช้จริง: เริ่มจาก GLOBAL_START_DATE หรือ 2 ปีล่าสุด
    try:
        kwargs = {'start': start_date} if start_date else {'period': '2y'}
        df = yf.Ticker(ticker).history(**kwargs)[['Close']].round(3)
        try:
...
def fetch_net_trades_since(asset_field_conf: Dict, window_start_bkk_iso: str, cache_bump: int = 0) -> int:
    try:
        ...
        params = {'start': _to_utc_z(window_start_local), 'timezone': 'UTC'}
...
def fetch_net_detailed_stats_between(asset_field_conf: Dict, window_start_bkk_iso: str, window_end_bkk_iso: str, cache_bump: int = 0) -> Dict[str, float]:
    try:
        ...
        params = {
            'start': _to_utc_z(window_start_local),
            'end': _to_utc_z(window_end_local),
            'timezone': 'UTC'
        }
        if asset_field_conf.get('api_key'):
            params['api_key'] = asset_field_conf.get('api_key')
...
def fetch_all_data(configs: List[Dict], _clients_ref: Dict, start_date: Optional[str],
    ...
    def fetch_monitor(asset_config: Dict) -> Tuple[str, Tuple[pd.DataFrame, str, Optional[datetime.date]]]:
        ...
        try:
            ...
            tickerData = get_history_df_close_bkk(ticker, start_date)
            if start_date:
