...
def get_ticker_data(ticker: str, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    """
    try:
        # ใช้ start และ end ที่ yfinance เข้าใจได้โดยตรง
        data = yf.Ticker(ticker).history(start=start_date, end=end_date)[['Close']]
        if data.empty:
            return pd.DataFrame()
        # แปลง timezone หลังจากดึงข้อมูลสำเร็จ
...
@lru_cache(maxsize=2048) # เพิ่มขนาด cache เพื่อรองรับการคำนวณที่หลากหลายขึ้น
def calculate_optimized_cached(action_tuple: Tuple[int, ...], price_tuple: Tuple[float, ...], fix: int = 1500) -> Tuple:
    ...
    # กำหนดให้ action แรกเป็น 1 เสมอในการคำนวณ ไม่ว่า input จะเป็นอะไร
    action_array[0] = 1
...
def generate_actions_perfect_foresight(prices: List[float], fix: int = 1500) -> np.ndarray:
    ...
    dp[0] = float(fix * 2) # Initial capital
...
def find_best_seed_for_window(prices_window: np.ndarray, num_seeds_to_try: int, max_workers: int) -> Tuple[int, float, np.ndarray]:
    ...
    batch_size = max(1, num_seeds_to_try // (max_workers * 4)) # Heuristic for batch size
                    ...
                    if not stitched_actions:
                        ...
                        with st.spinner(f"กำลังจำลองกลยุทธ์สำหรับ {stitch_ticker}..."):
                            ...
                            if sim_data.empty:
                                ...
                                # Run simulations for all three strategies on the new data
                                ...
                                plot_df = pd.DataFrame({
                                    'Max Performance (Perfect)': df_max['net'][:plot_len],
