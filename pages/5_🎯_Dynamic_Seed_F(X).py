...
def render_test_tab():
    ...
    if st.button("🚀 เริ่มทดสอบ Best Seed (Optimized)", type="primary"):
        ...
        with st.spinner("กำลังคำนวณกลยุทธ์ต่างๆ..."):
            ...
            results = {}
            # Use a more descriptive name for Rebalance Daily strategy
            strategy_map = {
                Strategy.SLIDING_WINDOW: actions_sliding.tolist(),
                "Rebalance Daily (Min)": actions_min.tolist(), # Changed from Strategy.REBALANCE_DAILY
                "Perfect Foresight (Max)": actions_max.tolist() # Changed from Strategy.PERFECT_FORESIGHT
            }
...
# ==============================================================================
# 4.1. <<< [FIXED] ADVANCED ANALYTICS TAB >>>
# ==============================================================================
def render_analytics_tab():
    """แสดงผล UI สำหรับ Tab 'Advanced Analytics Dashboard' (ฉบับสมบูรณ์)"""
                    ...
                    if not stitched_actions:
                        ...
                        with st.spinner(f"กำลังจำลองกลยุทธ์สำหรับ {stitch_ticker}..."):
                            ...
                            if sim_data.empty:
                                ...
                                df_max_sim = run_simulation(prices, generate_actions_perfect_foresight(prices).tolist())
                                df_min_sim = run_simulation(prices, generate_actions_rebalance_daily(n_total).tolist())

                                plot_len = len(df_dna)
                                if plot_len > 0:
                                    plot_df = pd.DataFrame({
                                        'Perfect Foresight (Max)': df_max_sim['net'].values[:plot_len],
                                        'Stitched DNA Strategy': df_dna['net'].values,
                                        'Rebalance Daily (Min)': df_min_sim['net'].values[:plot_len]
                                    ...
                                    final_net_min = df_min_sim['net'].iloc[plot_len-1]
                                    final_net_max = df_max_sim['net'].iloc[plot_len-1]
                                    
                                    metric_cols = st.columns(3)
                                    metric_cols[0].metric("Max Performance", f"${final_net_max:,.2f}")
                                    metric_cols[1].metric("Stitched DNA", f"${final_net_dna:,.2f}", delta=f"{final_net_dna - final_net_min:,.2f} vs Min", delta_color="normal")
...
# ==============================================================================
# 4.2. <<< [CORRECTED] MANUAL SEED TAB >>>
# ==============================================================================

def render_manual_seed_tab(config: Dict[str, Any]):
    """แสดงผล UI และจัดการ Logic สำหรับ Tab 'Manual Seed' (เวอร์ชัน Final)"""
    ...
    if st.button("📈 เปรียบเทียบประสิทธิภาพ Seeds", type="primary"):
        ...
        with st.spinner("กำลังดึงข้อมูลและจำลองการเทรด..."):
            ...
            if max_sim_len > 0:
                ...
                if not df_max.empty:
                    df_max.index = ticker_data.index[:max_sim_len]; results["Perfect Foresight (Max)"] = df_max
                if not df_min.empty:
                    df_min.index = ticker_data.index[:max_sim_len]; results["Rebalance Daily (Min)"] = df_min

            st.success("การเปรียบเทียบเสร็จสมบูรณ์!")
            display_comparison_charts(results, chart_title="📊 Performance Comparison (Net Profit)")

            st.subheader("สรุปผลลัพธ์สุดท้าย (Final Net Profit)")
            manual_strategy_names = [name for name in results.keys() if name not in ["Perfect Foresight (Max)", "Rebalance Daily (Min)"]]
            display_order = ["Perfect Foresight (Max)"] + sorted(manual_strategy_names) + ["Rebalance Daily (Min)"]
...
def main():
    ...
    st.caption("เครื่องมือทดสอบการหา Best Seed ด้วยวิธี Sliding Window (Refactored & Fixed Version)")
