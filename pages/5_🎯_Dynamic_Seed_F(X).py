...
# ==============================================================================
# 5. Analytics Dashboard Tab (Tap4)
# ==============================================================================
                    ...
                    if not stitched_actions:
                        ...
                        with st.spinner(f"กำลังจำลองกลยุทธ์สำหรับ {stitch_ticker}..."):
                            ...
                            if sim_data.empty:
                                ...
                                _, sumusd_dna, _, _, _, refer_dna = calculate_optimized_cached(tuple(final_actions_dna), tuple(prices[:len(final_actions_dna)]))
                                stitched_net = sumusd_dna - refer_dna - sumusd_dna[0]
                                max_actions = generate_actions_perfect_foresight(prices)
                                _, sumusd_max, _, _, _, refer_max = calculate_optimized_cached(tuple(max_actions), tuple(prices))
                                max_net = sumusd_max - refer_max - sumusd_max[0]
                                min_actions = np.ones(n_total, dtype=int).tolist()
                                _, sumusd_min, _, _, _, refer_min = calculate_optimized_cached(tuple(min_actions), tuple(prices))
