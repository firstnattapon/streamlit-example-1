...
class Strategy:
    ...
    MANUAL_SEED = "Manual Seed Strategy" # เพิ่มกลยุทธ์ใหม่
...
def initialize_session_state(config: Dict[str, Any]):
    ...
    # Session state สำหรับแท็บใหม่
    if 'manual_seed_lines' not in st.session_state:
        # ใส่ค่าเริ่มต้นตามตัวอย่างที่ให้มา
...
def calculate_optimized_cached(action_tuple: Tuple[int, ...], price_tuple: Tuple[float, ...], fix: int = 1500) -> Tuple:
    ...
    for i in range(1, n):
        ...
        if action_array_calc[i] == 0:
            amount[i] = amount[i-1]; buffer[i] = 0
        else:
...
def generate_actions_perfect_foresight(prices: List[float], fix: int = 1500) -> np.ndarray:
    ...
    for i in range(1, n):
        ...
        best_idx = np.argmax(current_sumusd); dp[i] = current_sumusd[best_idx]
...
def find_best_seed_for_window(prices_window: np.ndarray, num_seeds_to_try: int, max_workers: int) -> Tuple[int, float, np.ndarray]:
    ...
    if best_seed_for_window >= 0:
        ...
        best_seed_for_window = 1; best_actions = np.ones(window_len, dtype=int)
...
def generate_actions_sliding_window(
    ...
    for i, start_index in enumerate(range(0, n, window_size)):
        ...
        prices_window = prices[start_index:end_index]; window_len = len(prices_window)
...
# ==============================================================================
# 4. UI Rendering Functions (เพิ่มฟังก์ชันใหม่)
# ==============================================================================
...
def display_comparison_charts(results: Dict[str, pd.DataFrame], chart_title: str = '📊 เปรียบเทียบกำไรสุทธิ (Net Profit)'):
    ...
    for df in results.values():
        if not df.empty: main_index = df.index; break
    if main_index is None: st.warning("ไม่มีข้อมูลสำหรับสร้างกราฟเปรียบเทียบ"); return
...
def render_analytics_tab():
    ...
    if st.session_state.df_for_analysis is not None:
        st.markdown("ผลการวิเคราะห์"); df_to_analyze = st.session_state.df_for_analysis
        try:
            ...
            if not all(col in df_to_analyze.columns for col in required_cols):
                st.error(f"ไฟล์ CSV ไม่สมบูรณ์! กรุณาตรวจสอบว่ามีคอลัมน์เหล่านี้ทั้งหมด: {', '.join(required_cols)}"); return
            df = df_to_analyze.copy(); df['result'] = np.where(df['max_net'] > 0, 'Win', 'Loss')
            ...
            with overview_tab:
                ...
                if selected_window:
                    ...
                    st.markdown(f"**รายละเอียดของ Window #{selected_window}**"); w_cols = st.columns(3)
                    w_cols[0].metric("Net Profit", f"${window_data['max_net']:.2f}"); w_cols[1].metric("Best Seed", f"{window_data['best_seed']}")
                    w_cols[2].metric("Price Change", f"{window_data['price_change_pct']:.2f}%"); st.markdown("**Action Sequence:**")
                ...
                if st.button("🧬 เริ่มการวิเคราะห์ Stitched DNA", type="primary"):
                    if not stitched_actions: st.error("ไม่สามารถสร้าง Action Sequence จากข้อมูลที่โหลดได้")
                        ...
                        with st.spinner(f"กำลังจำลองกลยุทธ์สำหรับ {stitch_ticker}..."):
                            end_date = datetime.now(); sim_data = get_ticker_data(stitch_ticker, stitch_start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'))
                            if sim_data.empty: st.error("ไม่สามารถดึงข้อมูลสำหรับจำลองได้")
                            else:
                                prices = sim_data['Close'].tolist(); n_total = len(prices)
                                ...
                                if plot_len > 0:
                                    plot_df = pd.DataFrame({
                                        Strategy.PERFECT_FORESIGHT: df_max['net'].values[:plot_len],
                                        'Stitched DNA Strategy': df_dna['net'].values,
                                        Strategy.REBALANCE_DAILY: df_min['net'].values[:plot_len]
                                    ...
                                    final_net_dna = df_dna['net'].iloc[-1]; final_net_min = df_min['net'].iloc[plot_len-1]
                                ...
                                else: st.warning("ไม่มีข้อมูล DNA ที่จะแสดงผลได้")
        except Exception as e:
            st.error(f"เกิดข้อผิดพลาดในการวิเคราะห์ข้อมูล: {e}"); st.exception(e)

# ==============================================================================
# 4.5. <<< NEW FUNCTION FOR TAB 4 >>>
# ==============================================================================

def render_manual_seed_tab(config: Dict[str, Any]):
    """แสดงผล UI และจัดการ Logic สำหรับ Tab 'Manual Seed'"""
    st.header("🌱 Manual Seed Strategy Builder")
    st.markdown("สร้าง Action Sequence ด้วยตนเองโดยการกำหนด Seed, ขนาด และส่วนท้าย (Tail) ที่ต้องการนำมาต่อกัน")
    ...
    with st.container(border=True):
        ...
        with col1:
            ...
            manual_ticker = st.selectbox("เลือก Ticker", options=asset_list, index=default_index, key="manual_ticker")

        with col2:
            c1, c2 = st.columns(2)
            # ใช้ค่าจากตัวอย่างที่ให้มาเป็นค่าเริ่มต้น
            manual_start_date = c1.date_input("วันที่เริ่มต้น (Start Date)", value=datetime(2025, 4, 28), key="manual_start")
            manual_end_date = c2.date_input("วันที่สิ้นสุด (End Date)", value=datetime(2025, 6, 9), key="manual_end")
        ...
        st.write("**กำหนด Seed Lines ที่จะนำมาต่อกัน:**")

        for i, line in enumerate(st.session_state.manual_seed_lines):
            cols = st.columns([1, 2, 2, 2])
            cols[0].write(f"**Line {i+1}**")
            line['seed'] = cols[1].number_input("Input Seed", value=line['seed'], min_value=1, key=f"seed_{i}")
            line['size'] = cols[2].number_input("Size", value=line['size'], min_value=1, key=f"size_{i}")
            line['tail'] = cols[3].number_input("Tail", value=line['tail'], min_value=1, max_value=line['size'], key=f"tail_{i}")
        
        # --- Buttons to Add/Remove lines ---
        b_col1, b_col2, _ = st.columns([1,1,4])
        if b_col1.button("➕ เพิ่ม Line"):
            st.session_state.manual_seed_lines.append({'seed': 1, 'size': 10, 'tail': 5})
            st.rerun()
    ...
    # --- ปุ่มสำหรับเริ่มคำนวณ ---
    if st.button("🧬 สร้างและเปรียบเทียบผลลัพธ์", type="primary"):
        if manual_start_date >= manual_end_date:
            st.error("❌ กรุณาตั้งค่าช่วงวันที่ให้ถูกต้อง")
            return
        
        with st.spinner("กำลังสร้าง Action Sequence และจำลองการเทรด..."):
            # 1. สร้าง Final Actions จาก Inputs
            all_tail_actions = []
            for i, line_info in enumerate(st.session_state.manual_seed_lines):
                input_seed = line_info['seed']
                size_seed = line_info['size']
                tail_seed = line_info['tail']
                ...
                best_actions = rng_best.integers(0, 2, size=size_seed)
                tail_actions = best_actions[-tail_seed:]
                all_tail_actions.append(tail_actions)
            
            final_manual_actions = np.concatenate(all_tail_actions)
            
            # 2. ดึงข้อมูลราคา
            start_str = manual_start_date.strftime('%Y-%m-%d')
            end_str = manual_end_date.strftime('%Y-%m-%d')
            ...
            num_action_days = len(final_manual_actions)
            
            st.info(f"🧬 Action Sequence ที่สร้างขึ้นมีความยาว {num_action_days} วัน")
            st.info(f"📊 ข้อมูลราคามี {num_trading_days} วันทำการในช่วงที่เลือก")

            if num_action_days == 0:
                st.error("ไม่สามารถสร้าง Action ได้ กรุณาตรวจสอบค่า Input")
                return

            # กำหนดความยาวที่จะใช้จริง (ใช้ค่าน้อยสุดระหว่าง actions กับ prices)
            sim_len = min(num_trading_days, num_action_days)
            if sim_len == 0:
                st.warning("ไม่มีข้อมูลที่ซ้อนทับกันระหว่างช่วงวันที่และ actions ที่สร้างขึ้น")
                return

            prices_to_sim = prices[:sim_len]
            actions_to_sim = final_manual_actions[:sim_len].tolist()
            
            # 3. รัน Simulation
            df_manual = run_simulation(prices_to_sim, actions_to_sim)
            df_max = run_simulation(prices_to_sim, generate_actions_perfect_foresight(prices_to_sim).tolist())
            df_min = run_simulation(prices_to_sim, generate_actions_rebalance_daily(sim_len).tolist())

            results = {
                Strategy.MANUAL_SEED: df_manual,
                Strategy.PERFECT_FORESIGHT: df_max,
                Strategy.REBALANCE_DAILY: df_min
            }

            # 4. แสดงผลลัพธ์
            st.success("การจำลองเสร็จสมบูรณ์!")
            
            # กำหนด index ให้ตรงกับข้อมูลราคา
            for name, df in results.items():
                if not df.empty:
                    df.index = ticker_data.index[:sim_len]
            
            display_comparison_charts(results, chart_title="📊 Performance Comparison (Net Profit)")

            st.subheader("สรุปผลลัพธ์สุดท้าย (Final Net Profit)")
            final_net_manual = df_manual['net'].iloc[-1] if not df_manual.empty else 0
            final_net_min = df_min['net'].iloc[-1] if not df_min.empty else 0
            
            metric_cols = st.columns(3)
            metric_cols[0].metric(Strategy.PERFECT_FORESIGHT, f"${df_max['net'].iloc[-1]:,.2f}")
            metric_cols[1].metric(Strategy.MANUAL_SEED, f"${final_net_manual:,.2f}", delta=f"{final_net_manual - final_net_min:,.2f} vs Min", delta_color="normal")
            metric_cols[2].metric(Strategy.REBALANCE_DAILY, f"${final_net_min:,.2f}")

# ==============================================================================
# 5. Main Application (แก้ไขเพื่อเพิ่ม Tab 4)
# ==============================================================================
def main():
    ...
    # เพิ่มแท็บที่ 4 "Manual Seed"
    tab1, tab2, tab3, tab4 = st.tabs([
        "⚙️ การตั้งค่า", 
        "🚀 ทดสอบ", 
        "📊 Advanced Analytics",
        "🌱 Manual Seed" # แท็บใหม่
        ])
    ...
    # เรียกใช้ฟังก์ชันสำหรับแท็บที่ 4
    with tab4:
